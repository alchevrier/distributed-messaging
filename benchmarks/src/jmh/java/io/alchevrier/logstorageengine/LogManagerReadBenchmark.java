...
public class LogManagerReadBenchmark {
    ...
    public void setup() {
        this.logManager = new LogManagerImpl(
                "/tmp/benchmark/logManager/logs", 1000000, 50000, 1
        );
        this.priceOrder = providePriceOrder();
        this.topic = new Topic("test");
        numberOfMessages = 100000;
        currentReadOffset = 0;
        readBuffer = ByteBuffer.allocate(16);

        for (var i = 0; i < numberOfMessages; i++) {
            logManager.append(this.topic, null, this.priceOrder);
        }
    }
    ...
    public void priceAt(Blackhole bh) {
        if (currentReadOffset >= numberOfMessages) currentReadOffset = 0;
        bh.consume(logManager.read(this.topic, 0, currentReadOffset++, readBuffer));
    }
